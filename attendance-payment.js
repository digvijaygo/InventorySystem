(function () {
    ...
    function getInitialRecords() {
        return Array.from(tableBody.querySelectorAll('.employee-row')).map((row, index) => {
            ...
            const daily = Array.from({ length: daysInMonth() }, (_, dayIndex) => ({
                ...
                rate: statuses[dayIndex] ? rate : '', hours: statuses[dayIndex] === 'P' ? 8 : '',
                inTime: statuses[dayIndex] === 'P' ? '09:00' : '', outTime: statuses[dayIndex] === 'P' ? '17:00' : '',
                remarks: statuses[dayIndex] === 'L' ? 'Holiday' : ''
            }));
    ...
    function createEmployee(event) {
        ...
        records.push({ id: `EMP${String(records.length + 1).padStart(3, '0')}`, name: name.trim(), department: department.trim(), joiningDate, paid: 0, daily: Array.from({ length: daysInMonth() }, (_, index) => ({ date: isoDate(index + 1), status: '', rate: '', hours: '', inTime: '', outTime: '', remarks: '' })) });
    ...
    function normalizeMonthRecords() { records.forEach((record) => { const daily = Array.isArray(record.daily) ? record.daily : []; const defaultRate = daily.find((day) => day.rate)?.rate || ''; const joiningDate = record.joiningDate || isoDate(1); const daysByDate = new Map(daily.filter((day) => /^\d{4}-\d{2}-\d{2}$/.test(day.date)).map((day) => [day.date, day])); record.joiningDate = joiningDate; Array.from({ length: daysInMonth() }, (_, index) => isoDate(index + 1)).forEach((date) => { if (daysByDate.has(date)) return; const historicalDay = new Date(`${date}T00:00:00`) < new Date(new Date().toDateString()); const beforeJoining = date < joiningDate; const autoFill = !beforeJoining && historicalDay; daysByDate.set(date, { date, status: autoFill ? 'P' : '', rate: autoFill ? defaultRate : '', hours: autoFill ? 8 : '', inTime: autoFill ? '09:00' : '', outTime: autoFill ? '17:00' : '', remarks: '' }); }); record.daily = Array.from(daysByDate.values()).sort((first, second) => first.date.localeCompare(second.date)); }); }
